...
{ //Default constructor, just 0 everything.
    double mu1 = 0.0;
    double sigma1 = 0.0;
    double mu2 = 0.0;
    double sigma2 = 0.0;
...
std::vector<evt> ucn_gen_PCG::gen_evts(pcg64& r, std::vector<double> t0s)
{
    ...
    if ((numEvts % 2) == 1)
    { //If we had odd number of events generate last one
        double z1, z2;
        next2Norm(r, &z1, &z2);
        uint64_t u = r();
    ...
    for (int i = 0; i < numEvts; i++)
    {
        for (int j = 0; j < phs[i]; j++)
        {
            ...
            //p_pmt2*p_shrt, p_pmt2*p_med, p_pmt2*p_long. ch can be checked simply.
            //Then split by channel for determining tau.
            int ch = u < p_pmt1 ? 1 : 2;
            double tau = u < p_pmt1 ?
    ...
    {
        if (isinf(evts[i].t))
        { //Events which have been already deadtime'd don't generate more deadtime
            continue;
        }
        int j = i+1;
        while (j < evts.size() && (evts[j].t - evts[i].t) < 15e-9)
        { //While we have events which could be deadtime'd
            if (evts[j].ch == evts[i].ch)
            { //If the channels match, then event would not be seen.
